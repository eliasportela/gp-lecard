    ...
    if (newMessage && newMessage.isNewMsg && !newMessage.isSentByMe && !newMessage.isGroupMsg) {
      ...
      if (message) {
        ...
        if (user) {
          ...
          message = "Olá! bem-vindo ao nosso atendimento virtual." +
            "\n" +
            "\nAgilize seu pedido online" +
            "\nClique aqui: \n" +
            "$_CARDAPIO" +
            "\n\n" +
            "Aqui vai algumas dicas que você pode me perguntar" +
            "\nBasta escrever:" +
            "\n\n*Fazer um pedido*" +
            "\n*Onde posso fazer meu pedido?*";

          window.WappBot.prepareMessageToSend(window.WappBot.messageReplaceKey(message), chatId);
          users.push({
            chatId,
          });
